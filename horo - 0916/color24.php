...
<head>
  ...
  <meta property="og:url" content="https://evitadesignart.com/horo/color24.php" />
...
<div class="fv-buttons">
  ...
  <div id="modal" class="modal">
    <div class="modal-content">
      ...
      <div id="modal-share-buttons" style="margin-top: 20px; display: flex; justify-content: center; gap: 10px;"></div>
    </div>
  </div>
  <p>
...
<h2><a href="https://evitadesignart.com/horo/" style="display: block; width: 100%; text-align: center;"><button style="width: 100%; max-width: 600px;">占い一覧はコチラ</button></a></h2>
  ...
  <script>const colors = [
    ...
    function showModal(name, explanation, imgPath) {
      ...
      const position = explanation.startsWith("【逆位置】") ? "【逆位置】" : "【正位置】";
      let snippetText = (explanation || "").replace(/<[^>]*>/g, '').substring(0, 50) + "...";
      snippetText = snippetText.replace(/^【(正位置|逆位置)】\s*/g, '').replace(/^(正位置|逆位置)\s*/g, '');
      const hashPosition = position.replace(/【|】/g, '');
      const shareText = encodeURIComponent("#" + name + " #" + hashPosition + " #色占い \n" + snippetText + " | 占いアルカノヴァ");
      const shareUrl = encodeURIComponent("https://evitadesignart.com/horo/color24.php");
      const shareHtml = `
      ...
      `;
      document.getElementById("modal-share-buttons").innerHTML = shareHtml;
    }
