...
<body>
  ...
  <main>
    ...
    <div id="weekly-ranking-container">
        <h3>今週のタロット Best 3</h3>
        <div id="ranking-content" class="ranking-list">
            <p style="color: #ccc;">集計中...</p>
        </div>
        <a href="tarot_history.php" class="past-logs-btn">📅 過去のログ・世相占いを見る</a>
    </div>
    <script>
    document.addEventListener("DOMContentLoaded", function() {
        fetch('get_ranking.php?period=7 days')
            ...
            .then(data => {
                ...
                if (data.status === 'success' && data.data.length > 0) {
                    ...
                    data.data.forEach((item, index) => {
                        const rank = index + 1;
                        let rankLabel = rank + '位';
                        html += `
                            <div class="ranking-item">
                                <div class="ranking-rank">${rankLabel}</div>
                                <div class="ranking-name">${item.card_name}</div>
                                <div class="ranking-pos">【${item.position}】</div>
                                <div class="ranking-count">${item.count}回 引かれました</div>
                            </div>
                        `;
                    });
                    container.innerHTML = html;
                } else {
                    container.innerHTML = '<p style="color: #ccc;">まだデータがありません。</p>';
                }
            })
            .catch(err => {
                document.getElementById('ranking-content').innerHTML = '<p style="color: red;">読み込みエラー</p>';
            });
    });
    </script>
    <!-- End Weekly Tarot Ranking Section -->
