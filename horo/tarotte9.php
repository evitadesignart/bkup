...
<head>
  ...
  <meta property="og:description"
    content="無料で楽しめる大アルカナ2枚引きタロット占い。相手の本音、あなた自身の心を示し、関係性を深く読み解きます。恋愛や対人関係で悩んでいる方に、今必要な気づきとアドバイスをお届けします。" />
  <meta property="og:url" content="https://evitadesignart.com/horo/tarotte9.php" />
...
<body>
  ...
  <div id="fv-wrapper" class="fv-wrapper-custom">
    ...
    <div class="badge-container"><span class="badge-type">タロット占い</span><span class="badge-type">恋愛</span><span class="badge-type">現状</span><span class="badge-type">心理</span></div>
  </div>
  <script>
    (function(){
...
<h2><a href="https://evitadesignart.com/horo/" style="display: block; width: 100%; text-align: center;"><button style="width: 100%; max-width: 600px;">占い一覧はコチラ</button></a></h2>
  ...
  `;
      ...
      shareText = encodeURIComponent("#" + conflictCard.name + " #" + conflictPosition + " #" + harmonyCard.name + " #" + harmonyPosition + " #タロット占い \n" + snippetText + " | 占いアルカノヴァ");
      
            fetch('log_tarot.php', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({card_name: conflictCard.name.replace(/（.*$/, ''), position: conflictPosition})}); fetch('log_tarot.php', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({card_name: harmonyCard.name.replace(/（.*$/, ''), position: harmonyPosition})});
      const shareHtml = `
