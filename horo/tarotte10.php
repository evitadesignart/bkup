...
<head>
  <meta charset="UTF-8" />
  <title>自分のアルカナ診断｜誕生日タロットで導くバースタロット（無料） | 占いアルカノヴァ</title>
  ...
  <link rel="canonical" href="https://evitadesignart.com/horo/tarotte10.php" />
  ...
  <meta property="og:url" content="https://evitadesignart.com/horo/tarotte10.php" />
  ...
  <script type="application/ld+json">
  {
    ...
    "url": "https://evitadesignart.com/horo/tarotte10.php"
  }
  </script>
  <script type="application/ld+json">
{
...
<body>
  <header><a href="https://evitadesignart.com/horo/">
  ...
  <div id="fv-wrapper" class="fv-wrapper-custom">
    ...
    <div class="badge-container"><span class="badge-type">タロット占い</span><span class="badge-type">アルカナ診断</span><span class="badge-type">生年月日</span></div>
  </div>
  <script>
    (function(){
...
<div class="fv-buttons">
  ...
  <h2>タロットカードで<br class="md:hidden">導く、<br>あなたのアルカナと<br class="md:hidden">人生のテーマ</h2>
  <p>生年月日を入力してください</p>


  <form id="birthForm">
    <input type="date" value="2018-01-11" id="birthdate" required><br>
    <button type="submit" id="drawButton">生年月日から<br>バースタロット<br>(個人アルカナ)を<br>診断する</button>
  </form>
  ...
  <div class="sp-text-left"
    ...
    <strong style="color:#aabbff;">アルカナ診断の仕組み</strong>
    <p style="margin-top: 0.5em; margin-bottom: 0;">
      生年月日(西暦8桁)の数字をすべて足し合わせ、合計が22を超える場合はさらに各桁を足す、<br>という計算を繰り返して1～22の数字を導き出します。<br>
      数秘術で特別な数とされる「22」が出た場合は、大アルカナの0番目のカード「愚者」に対応します。<br>
      こうして生年月日から導かれた1枚が、あなたの生まれ持った本質やテーマを表す「バースタロット」です。<br><br>
      <strong>例:</strong> 1994年5月12日生まれの場合<br>
      1+9+9+4+0+5+1+2 = 31 → 3+1 = 4 → 「Ⅳ 皇帝」
    </p>
  </div>
...
<h2><a href="https://evitadesignart.com/horo/" style="display: block; width: 100%; text-align: center;"><button style="width: 100%; max-width: 600px;">占い一覧はコチラ</button></a></h2>
  ...
  <script>
    ...
    document.getElementById("birthForm").addEventListener("submit", function (event) {
      ...
      // try to extract name, if it's available. If it's a list, just use title.
      let shareTitle = document.title.split('|')[0].trim();
      let snippetText = meanings[cardIndex].replace(/<[^>]*>/g, '').substring(0, 50) + '...';
      snippetText = snippetText.replace(/^【(正位置|逆位置)】\s*/g, '').replace(/^(正位置|逆位置)\s*/g, '');
      const shareText = encodeURIComponent("#" + arcana[cardIndex] + " #アルカナ診断 #タロット占い \n" + snippetText + " | 占いアルカノヴァ");
            fetch('log_tarot.php', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({card_name: arcana[cardIndex], position: '正位置'})});
      const shareHtml = `
      ...
      `;
      if(typeof resultDiv !== 'undefined' && resultDiv) resultDiv.innerHTML += shareHtml;

      resultDiv.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  </script>

  <details style="margin: 2em auto; max-width: 640px; text-align:left; background:#2c2c54; padding:1em; border-radius:10px;">
    <summary style="cursor:pointer; color:#dcd6f7; font-weight:bold;">22枚の大アルカナ一覧(タップで開く)</summary>
    <div id="arcanaList" style="margin-top:1em; color:#dcd6f7; line-height:1.6;"></div>
  </details>
  <script>
    (function(){
      var listDiv = document.getElementById('arcanaList');
      if (listDiv && typeof arcana !== 'undefined') {
        var html = '';
        for (var i = 0; i < arcana.length; i++) {
          html += '<p><strong>' + i + '. ' + arcana[i] + '</strong><br>' + meanings[i] + '</p>';
        }
