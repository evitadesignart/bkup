...
document.addEventListener("DOMContentLoaded", function() {
  ...
  parallaxSections.forEach(section => {
    if (section.style.backgroundImage && section.style.backgroundImage !== 'none') {
      ...
      section.style.backgroundPosition = 'center 50%';
    }
  });
  ...
  function updateParallax() {
    parallaxSections.forEach(section => {
      if (section.style.backgroundImage && section.style.backgroundImage !== 'none') {
        ...
        // display: none; 等で高さが0の時は計算しない（エラー防止）
        ...
        if (rect.top < windowHeight && rect.bottom > 0) {
          ...
          const yPosPercent = progress * 100;
          section.style.backgroundPosition = `center ${yPosPercent}%`;
        }
      }
    });
  }
  ...
  if (navMenu) {
    ...
    // メニューが開かれた時に再計算するため、クリックイベント等も監視できるが
    // toggleMenu は HTML の onclick にあるので、MutationObserver で class 変化を監視
  ...
  // 初期化時に1回実行
  updateParallax();
  setTimeout(updateParallax, 500); // 念のため少し遅らせても実行
});
</script>
